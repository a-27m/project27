...
export const AVAILABLE_COLUMNS: readonly SheetColumn[] = [
  ...
  { key: 'row', label: 'ID', width: 44, render: (t) => String(t.row) },
  { key: 'uid', label: 'UID', width: 52, render: (t) => String(t.uid) },
  { key: 'wbs', label: 'WBS', width: 64, render: (t) => t.wbs },
  { key: 'name', label: 'Name', width: 260, render: (t) => t.name },
  { key: 'duration', label: 'Duration', width: 80, render: (t, c) => durationDays(t.durationMinutes, c.minutesPerDay, t.estimated) },
  { key: 'start', label: 'Start', width: 130, render: (t) => dateTime(t.start) },
  { key: 'finish', label: 'Finish', width: 130, render: (t) => dateTime(t.finish) },
  {
    key: 'predecessors',
    label: 'Predecessors',
    width: 110,
  ...
  { key: 'percentComplete', label: '%', width: 48, render: (t) => (t.percentComplete > 0 ? `${t.percentComplete}%` : '') },
  { key: 'work', label: 'Work', width: 70, render: (t) => (t.workMinutes > 0 ? `${Math.round((t.workMinutes / 60) * 10) / 10}h` : '') },
  { key: 'cost', label: 'Cost', width: 80, render: (t) => (t.cost !== 0 ? String(Math.round(t.cost * 100) / 100) : '') },
  {
    key: 'totalSlack',
    label: 'Slack',
    width: 70,
  ...
  { key: 'deadline', label: 'Deadline', width: 110, render: (t) => dateTime(t.deadline) },
  { key: 'constraint', label: 'Constraint', width: 130, render: (t) => (t.constraint === 'asSoonAsPossible' ? '' : t.constraint) },
] as const
